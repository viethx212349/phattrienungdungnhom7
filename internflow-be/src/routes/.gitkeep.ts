...
import { internService } from '../services/.gitkeep';

const router = Router();

// GET /api/interns - Lấy tất cả
router.get('/interns', async (req: Request, res: Response) => {
  try {
...
router.get('/interns/:id', async (req: Request, res: Response) => {
  try {
...
router.post('/interns', async (req: Request, res: Response) => {
  try {
...
router.put('/interns/:id', async (req: Request, res: Response) => {
  try {
    const id = parseInt(req.params.id as string);
    const { name, email } = req.body;
...
router.delete('/interns/:id', async (req: Request, res: Response) => {
  try {
    const id = parseInt(req.params.id as string);
